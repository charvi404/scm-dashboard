    ...
    if model_type == "Revenue Growth Prediction (Linear Regression)":
      ...
      if 'Supply_Chain_Agility' in df.columns:
        ...
        st.write("Predicts carbon emissions using Random Forest regression")
              # Carbon Emissions Prediction with Random Forest (Improved)
    if 'Carbon_Emissions_(kg_CO2e)' in df.columns:
        ...
        if len(existing_features) > 0:
            ...
            X_train_rf, X_test_rf, y_train_rf, y_test_rf = train_test_split(
                X_rf, y_rf, test_size=0.3, random_state=42)  # Changed to 30% test data
            ...
            with col1:
                st.metric("R² Score", f"{r2:.3f}")
                st.caption(f"Cross-validated R²: {np.mean(cv_scores):.3f}")
            with col2:
                st.metric("RMSE", f"{rmse:.3f}")
                st.caption(f"Data range: {y_rf.min():.0f} to {y_rf.max():.0f} kg CO2e")
            ...
            fig = px.scatter(
                ...
                labels={'x': 'Actual Emissions', 'y': 'Predicted Emissions'},
                title='Actual vs Predicted Carbon Emissions',
                trendline='ols'
            )
            fig.add_shape(type='line', x0=y_rf.min(), y0=y_rf.min(),
